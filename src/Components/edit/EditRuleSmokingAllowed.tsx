...
const EditRuleSmokingAllowed = (props: any) => {
    ...
    const handleChange = (smoking_allowed: number) => {
        setState({
            ...state,
            smoking_allowed
        })
    ...
    const onSubmit = async (e: any) => {
        ...
        const items = {
            safety_question: {
                smoking_allowed: state.smoking_allowed === 1,
            },
        }
        setLoading(true)
        try {
            ...
            {isExpended ?
                <div className="edit-input mt-2">
                    <div className="form-check mb-2">
                        <input className="form-check-input form-check-radio" type="radio" name="smoking_allowedRadioDefault" checked={state.smoking_allowed === 1} onChange={() => handleChange(1)} />
                        <label className="form-check-label" htmlFor="smoking">
                            Yes
                        </label>
                    </div>
                    <div className="form-check mb-4">
                        <input className="form-check-input form-check-radio" type="radio" name="smoking_allowedRadioDefault" checked={state.smoking_allowed === 0} onChange={() => handleChange(0)} />
                ...
                <div className="listing-content">
                    <p>{state.smoking_allowed ? 'Yes' : 'No'}</p>
                </div>
            }
        </form>
...
export default EditRuleSmokingAllowed